...
Lista *cria_lista(){
    ...
    for(i=0; i < MAX; i++){
        ...
        listinha->number[i].chave = CHAVE_BASE;
    }
...
int consulta_por_chave(Lista *listinha, Numero *number,int chave){
    ...
    for(i=0;i<listinha->tamanho;i++){
        if(listinha->number[i].chave == chave){

            *number = listinha->number[i];
            return 1;
        }
    }
...
void imprimir_lista(Lista *listinha){
    ...
    for (i = 0; i < listinha->tamanho; i++) {
        printf("Chave: %d, Valor: %d\n", 
               listinha->number[i].chave, listinha->number[i].valor);
    }
}
